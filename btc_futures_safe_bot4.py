...
TRADE_ALLOCATION_PCT = float(os.getenv("TRADE_ALLOCATION_PCT", "12"))
...
MIN_NET_PROFIT_PCT = float(os.getenv("MIN_NET_PROFIT_PCT", "0.05"))

# Entry filters
BUY_STREAK_MIN = int(os.getenv("BUY_STREAK_MIN", "5"))
MIN_SLOPE_PCT = float(os.getenv("MIN_SLOPE_PCT", "0.020"))
MIN_LONG_SLOPE_PCT = float(os.getenv("MIN_LONG_SLOPE_PCT", "0.030"))
MIN_ENTRY_EDGE_PCT = float(os.getenv("MIN_ENTRY_EDGE_PCT", "0.05"))
MAX_DROP_PROB_TO_BUY = float(os.getenv("MAX_DROP_PROB_TO_BUY", "30.0"))
MAX_OVEREXTENSION_PCT = float(os.getenv("MAX_OVEREXTENSION_PCT", "0.18"))

# Risk/profit protection
HARD_STOP_PCT = float(os.getenv("HARD_STOP_PCT", "0.18"))
ARM_PROFIT_LOCK_PCT = float(os.getenv("ARM_PROFIT_LOCK_PCT", "0.08"))
PROFIT_GIVEBACK_PCT = float(os.getenv("PROFIT_GIVEBACK_PCT", "12.0"))
...
MAX_FLAT_HOLD_SECONDS = int(os.getenv("MAX_FLAT_HOLD_SECONDS", "120"))
LOSS_COOLDOWN_SECONDS = int(os.getenv("LOSS_COOLDOWN_SECONDS", "45"))

PRICE_BUFFER_SIZE = int(os.getenv("PRICE_BUFFER_SIZE", "300"))
...
TRADES_FILE = DATA_DIR / "futures_safe_trades.jsonl"
STATUS_FILE = DATA_DIR / "futures_safe_status.json"
...
class BTCFuturesSafeBot:
    ...
    def set_loss_cooldown(self) -> None:
        self.cooldown_until_ts = max(self.cooldown_until_ts, now_ts() + LOSS_COOLDOWN_SECONDS)
    ...
    def should_sell(self) -> tuple[bool, str]:
        ...
        # if not yet profitable enough, stay only if structure is still healthy
        if not sell_ok:
            if self.brain.sell_streak >= 2 and self.brain.drop_probability_pct() >= 60:
        ...
        if self.brain.sell_streak >= SELL_STREAK_MIN and self.brain.drop_probability_pct() >= 50:
    ...
    def print_status(self) -> None:
        ...
        print(
            ...
            f"cooldown={self.cooldown_left()}s | reason={self.last_reason}"
        )
...
async def run_bot() -> None:
    ...
    print(f"Starting futures-safe bot v4 | SYMBOL={SYMBOL} | INST_TYPE={INST_TYPE}")
