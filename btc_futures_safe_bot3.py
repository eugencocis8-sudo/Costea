...
TRADE_ALLOCATION_PCT = float(os.getenv("TRADE_ALLOCATION_PCT", "20"))
LEVERAGE = float(os.getenv("LEVERAGE", "1"))

# Futures fees (default taker/taker)
...
MIN_NET_PROFIT_PCT = float(os.getenv("MIN_NET_PROFIT_PCT", "0.03"))

# Entry filters
BUY_STREAK_MIN = int(os.getenv("BUY_STREAK_MIN", "4"))
MIN_SLOPE_PCT = float(os.getenv("MIN_SLOPE_PCT", "0.015"))
MIN_ENTRY_EDGE_PCT = float(os.getenv("MIN_ENTRY_EDGE_PCT", "0.04"))
MAX_DROP_PROB_TO_BUY = float(os.getenv("MAX_DROP_PROB_TO_BUY", "35.0"))

# Risk/profit protection
HARD_STOP_PCT = float(os.getenv("HARD_STOP_PCT", "0.25"))
ARM_PROFIT_LOCK_PCT = float(os.getenv("ARM_PROFIT_LOCK_PCT", "0.06"))
PROFIT_GIVEBACK_PCT = float(os.getenv("PROFIT_GIVEBACK_PCT", "18.0"))
...
PRICE_BUFFER_SIZE = int(os.getenv("PRICE_BUFFER_SIZE", "240"))
...
class BTCFuturesSafeBot:
    ...
    def save_status(self) -> None:
        ...
        snap = Snapshot(
            ...
            status="HOLD" if not self.wallet.has_position() else "LONG",
    ...
    def should_buy(self) -> tuple[bool, str]:
        ...
        if len(self.brain.prices) < 16:
            return False, "Not enough ticks"

        slope = self.brain.slope_pct(14)
    ...
    def should_sell(self) -> tuple[bool, str]:
        ...
        if self.brain.fast_dump_detected() and self.brain.sell_streak >= max(2, SELL_STREAK_MIN - 1):
        ...
        if self.brain.sell_streak >= SELL_STREAK_MIN and self.brain.drop_probability_pct() >= 54:
    ...
    def print_status(self) -> None:
        ...
        print(
            ...
            f"Eq={self.wallet.equity(self.current_price):.2f} | slope={self.brain.slope_pct():+.3f}% | "
...
async def run_bot() -> None:
    ...
    print(f"Starting futures-safe bot | SYMBOL={SYMBOL} | INST_TYPE={INST_TYPE}")
